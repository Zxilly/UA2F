...
static int queue_cb(const struct nlmsghdr *nlh, void *data)
{
    ...
    if (tb[NFQA_MARK]) {
        printf("mark=%u ", ntohl(mnl_attr_get_u32(tb[NFQA_MARK])));
    } else {
        printf("no mark");
    }
...
static struct nlmsghdr *nfq_build_verdict(char *buf, int id, int queue_num, int verd)
{
    ...
    mnl_attr_put_u32(nlh,NFQA_MARK,htonl(234));
...
int main(int argc, char *argv[])
{
    ...
    nlh = nfq_build_cfg_request(buf, NFQNL_CFG_CMD_BIND, (int)queue_num);
    ...
    nlh = nfq_build_cfg_params(buf, NFQNL_COPY_PACKET, 0xFFFF, (int)queue_num);
    ...
    while (ret > 0) {
        ...
        id = ret - MNL_CB_OK;
        nlh = nfq_build_verdict(buf, id, (int)queue_num, NF_ACCEPT);
        if (mnl_socket_sendto(nl, nlh, nlh->nlmsg_len) < 0) {
            perror("mnl_socket_sendto");
            exit(EXIT_FAILURE);
        }
